    ...
    private static ConsumerRebalanceListener createListener(KafkaConsumer<String, String> consumer) {
        return new ConsumerRebalanceListener() {
            ...
            public void onPartitionsRevoked(Collection<TopicPartition> collection) {
                System.out.println("### On paartitions revoked start ###");
            }
                ...
                partitions.forEach(partition -> {
                    try {
                        if (Files.exists(Paths.get(OFFSET_FILE_PREFIX + partition.partition()))) {
                            long offset = Long.parseLong(Files.readAllLines(Paths.get(OFFSET_FILE_PREFIX +
                                    partition.partition()), Charset.defaultCharset()).get(0));
                            consumer.seek(partition, offset);
                        }
                    } catch (IOException e) {
                        System.out.println("ERR: Could not read offset from file.  ###");
                    }
                });
            }
        };
